    ...
    public function searchMateriel($searchTerm)
    {
        try {
            ...
            if (!empty($searchTerm)) {
                ...
                $stmt = $this->db->prepare("SELECT * FROM Materiel WHERE description_materiel LIKE ? OR nom_utilisateur LIKE ? OR prenom_utilisateur LIKE ? OR type_materiel LIKE ? OR emplacement_materiel LIKE ? OR annee_materiel LIKE ? OR service_utilisateur LIKE ?");
